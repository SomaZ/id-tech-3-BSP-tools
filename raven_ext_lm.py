...
def main():
	...
	if bsp.header.magic_nr.decode('ASCII') != "RBSP":
		print("File is no RBSP", map_name)
		print("Press Key to quit")
		input()
		return
	...
	nomip_shader_names = [
		"maps/{}/nomipmaps{}".format(map_name, i).lower() for i in range(
	...
	if shader_file is not None:
		...
		open_shader = 0
		for line in lines:
			# remove all previous lightmap entries
			if line.strip().lower().startswith("map $lightmap"):
				continue
			# add map $lightmap before the external lightmaps
			if line.strip().lower().startswith("map maps/{}/lm_".format(map_name)):
				# only if its not a nomip shader
				if open_shader == 0:
					new_shader_file_lines.append("\t\tmap $lightmap")
			# store shader names for shaders with external lightmaps
			if open_shader == 0 and not line.strip().startswith("//") and len(line.strip()) > 0 and line.strip().startswith(map_name):
				ext_lm_shaders.append(line)
			# shader name
	...
	# Add new nomip and notc shaders
	for id, shader_name in enumerate(nomip_shader_names):
		...
		new_shader_file_lines.append("\tnotc")
	...
	# Find existing nomip shader entries
	bsp_nomip_shaders = [
		shader.name.decode() for shader in bsp.lumps["shaders"]
		if shader.name.decode().startswith("maps/{}/nomipmaps".format(map_name))]
	print("Existing nomip shaders in bsp", bsp_nomip_shaders)

	# Add shaders to the bsp and create new surfaces
	# TODO: skip existing nomipmaps shaders and reuse
	new_surface_lump = []
	for shader_name in nomip_shader_names:
		if shader_name in bsp_nomip_shaders:
			continue
	...
	# Add dummy lightmap, else engine will allways fallback to vert lighting
	new_lm = bsp.lump_info["lightmaps"]()
	bsp.lumps["lightmaps"] = [new_lm]

	# Set exteral lightmapped shader surfaces to use dummy lightmap
	ext_lm_shader_ids = [i for i, shader in enumerate(bsp.lumps["shaders"]) if shader.name.decode() in ext_lm_shaders]
	print("External lightmap shaders:", ext_lm_shaders)
	for surf in bsp.lumps["surfaces"]:
		if surf.texture in ext_lm_shader_ids:
			surf.lm_indexes[0] = 0

	new_bsp_bytes = bsp.to_bytes()
	try:
		new_file_name = import_settings.file[:-(len(".bsp"))]+"_ext_lm.bsp"
